...
OUTPUT_PATH = r'C:\\Work\\Tracker\\Marquee_Students.xlsx'
...
def main():
    print('Loading source and template workbooks...')
    ...
    marquee_map = {}
    for col_idx, name, _ in marquee_headers:
        key = normalize(name)
        if key and key not in marquee_map:
            marquee_map[key] = col_idx

    # Synonyms to match common alternative names
    synonyms = {
        'registrationnumber': ['registerno', 'regno', 'registrationno', 'regnumber', 'regdno', 'regid'],
        'geeksforgeekslink': ['gfglink', 'geeksforgeeks', 'gfgurl', 'geeksforgeeksurl', 'gfgprofile', 'gfg'],
        'leetcodelink': ['leetcode', 'leetcodeurl', 'leetcodeprofile', 'leetcodeid'],
        'codecheflink': ['codechef', 'codechefurl', 'codechefprofile'],
        'name': ['studentname', 'fullname', 'full_name', 'candidate', 'applicant']
    }
    ...
    # Header: copy text, styles, and column widths from sample
    for tgt_pos, (s_col_idx, header_text, sample_cell) in enumerate(sample_headers, start=1):
        cell = ws_out.cell(row=1, column=tgt_pos, value=header_text)
        ...
        ws_out.column_dimensions[col_letter_tgt].width = width if width is not None else 15

    # Determine number formats for data columns based on sample (first non-empty below header)
    column_num_formats = {}
    for tgt_pos, (s_col_idx, _, _) in enumerate(sample_headers, start=1):
        fmt = None
        for r in range(sample_header_row + 1, min(ws_s.max_row, sample_header_row + 50) + 1):
            ...
            if sc.value is not None and str(sc.value) != '':
                ...
                break
        column_num_formats[tgt_pos] = fmt or 'General'

    # Build mapping from sample headers -> marquee columns (with synonyms)
    mapping = []
    ...
    # Copy data rows from marquee into output according to mapping
    start_row_src = marquee_header_row + 1
    start_row_tgt = 2
    rows_written = 0
    for r in range(start_row_src, ws_m.max_row + 1):
        ...
        for tgt_pos, _, src_col in mapping:
            c = ws_out.cell(row=tgt_r, column=tgt_pos)
            if src_col:
                c.value = ws_m.cell(row=r, column=src_col).value
    ...
    if sample_headers:
        ...
        ws_out.auto_filter.ref = f'A1:{end_col_letter}1'

    wb_out.save(OUTPUT_PATH)
    print(f'Saved {rows_written} row(s) to {OUTPUT_PATH}')
